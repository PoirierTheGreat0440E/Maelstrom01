...
export default function Pagus({params}){
    ...
    const lectureMessage = function(portion){
        ...
        axios.get("http://127.0.0.1:1234/message",{ params:{ idThread:params.threadId,boardName:"bla",limite: portion} } ).then(function(res){  setLSmessages( res.data ); setNBmessage(res.data.length); setLecture(false); }).catch(function(err){console.log(err);});
    ...
    return(
    <div className="partie_bla">
    <main className="contenu">

        <Formulaire2 destination={params.threadId} board="bla"/>
