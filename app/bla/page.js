...
export default function Etude(){
    ...
    const listerThreads = function(){
        axios.get("http://127.0.0.1:1234/threads",{params:{boardName:"bla"}}).then(function(res){setLSthreads(res.data);}).catch(function(err){console.log(err);});
...
function ThreadElement({id,titre,date}){
    ...
    return(
    <Link className="THREAD" href={"bla/"+id}>
