...
export default function Pagus({params}){
    ...
    const lectureMessage = function(portion){
        ...
        axios.get("http://127.0.0.1:1234/message",{ params:{ idThread:params.threadId,boardName:"inf",limite: portion} } ).then(function(res){  setLSmessages( res.data ); setNBmessage(res.data.length); setLecture(false); }).catch(function(err){console.log(err);});
    ...
    return(
    <div className="partie_inf">
    <main className="contenu">

        <Formulaire2 destination={params.threadId} board="inf"/>
...
function MessageElement({date,fichiers,contenu,cle}){
    ...
    return(<div className="message_element">
        ...
        {array_fichiers.map(
            ...
            fi => <>{array_imagos.includes(fi.split(".")[1]) ? <Link key={cle} href={"/uploads/"+fi} target="_blank" ><Image key={cle} src={"/uploads/"+fi} alt={"/uploads/"+fi}  width={200} height={250}  placeholder='empty' /></Link> : fi.split(".")[1] == "mp4" ? <video controls><source src={"/uploads/"+fi} type="video/mp4"/></video> : fi.split(".")[1] == "mp3" ? <audio controls><source src={"/uploads/"+fi}/></audio> : <Link href={"/uploads/"+fi} target="_blank" >{fi}</Link>} </>
